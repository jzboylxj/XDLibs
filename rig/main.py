...
def yellow_component(name="", shape_type="", translate=(0, 0, 0), parent_node=None, have_loc=False):
    u"""黄色的基础控制器组件
...
def jnt_or_control_grp(name, object_type="joint", parent_node=None, have_loc=False):
    u"""骨骼组或控制器组

    :param name:
    :param object_type: 可选项：joint,plane,tours,locator
    :param parent_node:
    :param have_loc:
    :return: grp
    """
    ...
    def build_base_ctrl_group(self):
        for side in ["LF", "RT"]:
            ...
            for item in ["Up", "Low", "Inner", "Outer"]:
                ...
                mp_node = "{}_{}Master_MP".format(prefix, item)
                ...
                pm.PyNode(out_curve).getShape().attr('worldSpace[0]').connect(pm.PyNode(mp_node).attr('geometryPath'))
                pm.PyNode(mp_node).attr("allCoordinates").connect(pm.PyNode(base_ctrl_grp).translate)
    ...
    def build_base_loc_group(self):
        for side in ["LF", "RT"]:
            ...
            for item in ["Up", "Low", "Inner", "Outer"]:
                ...
                rotate = pm.PyNode(loc_grp).rotate.get()
                pm.PyNode(loc_grp).rotate.set(rotate[0], rotate[1] * -1, rotate[2])
...
class BrowCreator(Creator):
    ...
    def output_follicle(self):
        for side in ["LF", "RT"]:
            ...
            for index in range(0, self.brow_segment):
                ...
                # 添加次级控制器到毛囊的子层级
                self.sub_control(side=side, index=index +
                                                  1, parent_node=follicle)
        return True
    ...
    def init_structure(self):
        ...
        for side in ["LF", "RT"]:
            ...
            side_output_jnt_grp = "{}_{}_Sub_Output_Jnt_Grp".format(
                side, self.module_name)
            if not pm.objExists(side_output_jnt_grp):
                pm.createNode(
                    "transform", name=side_output_jnt_grp, p=side_module_grp)
    ...
    def master_sub_control(self):
        control_num = 4
        for side in ["LF", "RT"]:
            master_ctrl = "{}_{}_Master_Ctrl".format(
                side, self.module_name)
            ...
            for index in range(0, control_num):
                if index < 1:
                    ...
                    pm.PyNode(temp_mp_node).attr("uValue").set(
                        float(index) / (float(control_num - 1)))
                control_name = "{}_{}_Main_{}_Ctrl".format(
                    side, self.module_name, "{0:02d}".format(index + 1))
                control_grp = control_grp_have_joint(
                    name=control_name, parent_node=master_ctrl)
                pm.delete(pm.parentConstraint(temp_loc, control_grp,
                                              mo=False, skipRotate=["x", "z"]))
                if side == "RT":
    ...
    def local_rig_out(self):
        ...
        for side in ["LF", "RT"]:
            ...
            for index in range(1, seg_of_brow + 1):
                sub_null = "{}_Sub_{}_Null".format(
                    prefix, "{0:02d}".format(index))
                pm.createNode("transform", name=sub_null, p=rig_track_grp)
                sub_ctrl = "{}_Sub_{}_Ctrl".format(
                    prefix, "{0:02d}".format(index))
                pm.parentConstraint(sub_ctrl, sub_null, mo=False)
                pm.scaleConstraint(sub_ctrl, sub_null, mo=False)
...
class NoseCreator(Creator):
    ...
    def proxy(self):
        ...
        for item in ["Bridge", "Master", "MD", "LF", "RT", "Up"]:
            pm.parent(pm.spaceLocator(
                name="proxyNose{}Loc".format(item)), "Proxy_Nose_Grp")
        ...
        pm.parent("proxyNoseMDLoc", "proxyNoseLFLoc",
                  "proxyNoseRTLoc", "proxyNoseMasterLoc")
    ...
    def local_rig_out_grp(self):
        ...
        if not pm.objExists(local_rig_out_grp):
            pm.createNode("transform", name=local_rig_out_grp,
                          p="{}_Deformer_Grp".format(mid_prefix))

        bridge_jnt_grp = jnt_or_control_grp(name="{}_Bridge_Ctrl_Jnt".format(
            mid_prefix), parent_node=local_rig_out_grp)
        pm.delete(
            pm.parentConstraint(self.proxy_bridge, bridge_jnt_grp, mo=False))
        ...
        pm.delete(pm.parentConstraint(
            self.proxy_bridge, bridge_ctrl_grp, mo=False))
        ...
        left_ctrl_grp = yellow_component(
            ...
            parent_node="{}_Master_Ctrl".format(mid_prefix))
        pm.delete(pm.parentConstraint(
            self.proxy_left, left_ctrl_grp, mo=False))
        ctrl_list.append("{}_Ctrl".format(lf_prefix))

        right_jnt_grp = yellow_component(
            name="{}_Ctrl_Jnt".format(rt_prefix),
            shape_type="joint",
            parent_node="{}_Master_Ctrl_Jnt".format(mid_prefix))
        pm.delete(pm.parentConstraint(
            self.proxy_right, right_jnt_grp, mo=False))
        right_ctrl_grp = yellow_component(
            name="{}_Ctrl".format(rt_prefix),
            shape_type="sphere",
            parent_node="{}_Master_Ctrl".format(mid_prefix))
        pm.delete(pm.parentConstraint(
            self.proxy_right, right_ctrl_grp, mo=False))
    ...
    def build_deformer_grp(self):
        ...
        for side in ["LF", "RT"]:
            ...
            if not pm.objExists(cheek_jnt_group):
                pm.createNode("transform", name=cheek_jnt_group,
                              p=deformer_grp)
            ...
            if not pm.objExists(control_main_grp):
                pm.createNode(
                    "transform", name=control_main_grp, p=control_grp)

            for index in range(1, cheeck_num + 1):
                cheek_jnt = "{}_{}_Ctrl_Jnt".format(
                    prefix, "{0:02d}".format(index))
                cheek_jnt_grp = yellow_component(
                    name=cheek_jnt,
                    shape_type="joint",
                    parent_node=cheek_jnt_group)
                proxy_loc = "{}_proxyCheek{}Loc".format(
                    side, "{0:02d}".format(index))
                pm.delete(pm.parentConstraint(
                    proxy_loc, cheek_jnt_grp, mo=False))

                cheek_ctrl = "{}_{}_Ctrl".format(
                    prefix, "{0:02d}".format(index))
                cheek_ctrl_grp = yellow_component(
                    name=cheek_ctrl,
                    shape_type="sphere",
                    parent_node=control_main_grp)
                pm.delete(pm.parentConstraint(
                    proxy_loc, cheek_ctrl_grp, mo=False))

                pm.PyNode(cheek_ctrl).translate.connect(
                    pm.PyNode(cheek_jnt).translate)
                pm.PyNode(cheek_ctrl).rotate.connect(
                    pm.PyNode(cheek_jnt).rotate)
...
class MouthCreator(Creator):
            ...
            for side in ["LF", "RT"]:
                ...
                if location == "Up":
                    ...
                    pm.PyNode("MD_{}_Tweak_Ori_Null".format(self.module_name)).rotate.connect(
                        pm.PyNode(ctrl_grp).rotate)
                    pm.PyNode("MD_{}_Scale_Null".format(self.module_name)
                              ).scale.connect(pm.PyNode(ctrl_grp).scale)
    ...
    def use_dm_node_to_tweak_ctrl_parent_and_jnt_02_grp(self):
        ...
        for side in ["LF", "RT"]:
            ...
            custom_show_channel(tweak_ctrl, attr_list=[
            ...
            dm_node = pm.createNode(
                "decomposeMatrix", name="{}_Inverse_DM".format(tweak_ctrl))
            ...
            for location in ["Up", "Low"]:
                for index in range(1, seg + 1):
                    ...
                    jnt_02_grp = (
                        "{}_Mouth_01_{}Tweak_{}_Ctrl_Jnt_02_Grp".format(side, location, "{0:02d}".format(index)))
                    pm.PyNode(tweak_ctrl).translate.connect(
                        pm.PyNode(jnt_02_grp).translate)
                    pm.PyNode(tweak_ctrl).attr("roll").connect(
                        pm.PyNode(jnt_02_grp).rotateX)
        ...
        for side in ["LF", "RT"]:
            ...
            if not pm.objExists(lip_sew_ctrl):
                lip_sew_ctrl_grp = yellow_component(
                    name=lip_sew_ctrl, shape_type="cone", parent_node=follicle, )
    ...
    def lip_sew_and_follicle(self):
        ...
        if not pm.objExists(sew_ctrl_grp):
            pm.createNode("transform", name=sew_ctrl_grp,
                          p="MD_{}_Grp".format(self.module_name))
        ...
        for side in ["LF", "RT"]:
            ...
            pm.PyNode("MD_{}_Scale_Null".format(self.module_name)
                      ).scale.connect(pm.PyNode(follicle).scale)
            pm.PyNode(follicle).getShape().attr("parameterV").set(0.5)
    ...
    def proxy_jaw(self, have_teeth=True):
        ...
        if not pm.objExists(jaw_deform_grp):
            pm.createNode("transform", name=jaw_deform_grp,
                          p="MD_{}_Deformer_Grp".format(self.module_name))
    ...
    def jaw_deformer_grp(self):
        ...
        if not pm.objExists(jaw_out_grp):
            pm.createNode("transform", name=jaw_out_grp,
                          p="MD_{}_Deformer_Grp".format(self.module_name))

        low_jnt = "MD_Mouth_01_LowJaw_Jnt"
        low_jnt_grp = jnt_or_control_grp(
            name=low_jnt, parent_node=jaw_out_grp, have_loc=True)
        tweak_jnt_grp = jnt_or_control_grp(
            name="MD_Mouth_01_Jaw_Tweak_Ctrl_Jnt", parent_node="MD_Mouth_01_LowJaw_Jnt")
        low_teeth_jnt_grp = jnt_or_control_grp(name="MD_Mouth_01_Low_Teeth_Ctrl_Jnt",
        ...
        pm.delete(pm.parentConstraint(
            "proxyTeethLow", low_teeth_jnt_grp, mo=False))
        pm.delete(pm.parentConstraint("proxyJawUp", up_jnt_grp, mo=False))
        pm.delete(pm.parentConstraint(
            "proxyTeethUp", up_teeth_jnt_grp, mo=False))
        ...
        pm.delete(pm.parentConstraint("Up_{}_Ctrl".format(
            self.module_name), up_null_grp, mo=False))
        low_null_grp = ctrl_null_grp(
            name="Low_Mouth_01_Ctrl_Null", parent_node=jaw_out_grp)
        pm.delete(pm.parentConstraint(
            "Low_{}_Ctrl".format(self.module_name), low_null_grp, mo=False))
        lf_null_grp = chain_ctrl_null_grp(
            name="LF_Mouth_01_Ctrl_Null", parent_node=jaw_out_grp)
        pm.delete(pm.parentConstraint("LF_{}_Ctrl".format(
            self.module_name), lf_null_grp, mo=False))
        rt_null_grp = chain_ctrl_null_grp(
            name="RT_Mouth_01_Ctrl_Null", parent_node=jaw_out_grp)
        pm.delete(pm.parentConstraint("RT_{}_Ctrl".format(
            self.module_name), rt_null_grp, mo=False))
    ...
    def jaw_control_grp(self):
        ...
        if not pm.objExists(jaw_ctrl):
            pm.delete(
                pm.parentConstraint(
                    "proxyJawRoot",
                    jnt_or_control_grp(
                        name=jaw_ctrl, object_type="plane", parent_node="MD_{}_Grp".format(self.module_name)),
                    mo=False))
        ...
        pm.delete(
            pm.parentConstraint(
                "proxyJawTeak",
                jnt_or_control_grp(
                    name=tweak_ctrl, object_type="plane", parent_node=jaw_ctrl),
                mo=False))
        ...
        pm.delete(
            pm.parentConstraint(
                "proxyTeethLow",
                jnt_or_control_grp(name=low_teeth_ctrl,
                                   object_type="plane", parent_node=jaw_ctrl),
                mo=False))
...
class FaceCreatorUI(common.Singleton):
    ...
    def show_window(self):
        ...
        self.eye_module_frame(eye_ear_tab)
    ...
    def eye_module_frame(self, parent):
        frame = pm.frameLayout(
            p=parent, label="Preparatory Work", mh=5, mw=10)

        pm.frameLayout(label=u"Guid Curve", ann=u"这根曲线将会约束眼皮的蒙皮骨骼",
                       mh=10, mw=10, bgs=True, cll=True, cl=True)
        ...
        pm.frameLayout(label=u"Control location", mh=10,
                       mw=10, bgs=True, cll=True, cl=False)
        ...
        pm.button(label=u"Build Module",
                  c=lambda *args: self.build_eye_module())
    ...
    def brow_module_frame(self, parent):
        ...
        pre_frame = pm.frameLayout(
            p=frame, label=u"Components", mh=10, mw=10, bgs=True)
        pm.textFieldButtonGrp(
        ...
        parameter_frame = pm.frameLayout(
            p=frame, label=u"Parameters", mh=5, mw=10, bgs=True)
        pm.intFieldGrp("xdMouthCreatorBrowSubSegmentField",
                       label=u"Segment", numberOfFields=1, value1=7)
        pm.setParent(parameter_frame)

        pm.button(label="Build module", p=frame,
                  c=lambda *args: self.build_brow_module())
        pm.setParent(frame)
    ...
    def nose_module_frame(self, parent):
        ...
        pm.button(label=u"Build Module",
                  c=lambda *args: self.build_nose_module())
    ...
    def cheek_module_frame(self, parent):
        ...
        pm.button(label=u"Build Module", p=frame,
                  c=lambda *args: self.build_cheek_module())
        pm.setParent(frame)
        return frame

    def eye_proxy_frame_init(self):
        for side in ["LF", "RT"]:
            if side == "LF":
                sider = "Left"
            else:
            ...
            for vertical in ["Up", "Low"]:
                ...
                for item in ["Master", "Blink", "Out", "Tweak", "Freshy"]:
                    ...
                    if pm.objExists(curve):
                        pm.textFieldButtonGrp(
                            "xdMouthCreator{}{}{}CurveField".format(sider, vertical, item), e=True, text=curve)
                surface = "{}_Shape_Surface".format(prefix)
                # print("surface:{}".format(surface))
                if pm.objExists(surface):
                    pm.textFieldButtonGrp(
                        "xdMouthCreator{}{}ShapeSurfaceField".format(sider, vertical), e=True, text=surface)

            master_ctrl = "{}_{}_Master_Ctrl".format(side, self.eye_module)
            if pm.objExists(master_ctrl):
                pm.textFieldButtonGrp("xdMouthCreator{}MasterCtrlField".format(
                    sider), e=True, text=master_ctrl)
    ...
    def nose_proxy_frame_init():
        for item in ["Bridge", "Master", "MD", "LF", "RT", "Up"]:
            ...
            if pm.objExists(proxy_loc):
                pm.textFieldButtonGrp("xdMouthCreatorNose{}ProxyField".format(
                    item), e=True, text=proxy_loc)
        return
    ...
    def cheek_proxy_frame_init(self):
        for side in ["LF", "RT"]:
            for index in range(0, 5):
                ...
                if pm.objExists(proxy_loc):
                    pm.textFieldButtonGrp(
                        "xdMouthCreator{}Cheek{}ProxyField".format(
                            sider, "{0:02d}".format(index + 1)),
    ...
    def eye_master_ctrl_field(self, side, field):
        ...
        master_ctrl = "{}_Master_Ctrl".format(prefix)
        jnt_or_control_grp(name=master_ctrl, object_type="tours",
                           parent_node="{}_Grp".format(prefix))
